...
what actually blocked a common-code version, how the shadow seam works, and how the
60 fps claim was verified on a real device.
...
Both recordings show the sample app in this repo running the same `commonMain` composable.
...
Compose) consumed by both platform apps. It includes an on-screen FPS readout, which helps
when judging curl smoothness on a real device:
...
derivative work as a whole under different terms, and its conditions are honored here: the upstream
